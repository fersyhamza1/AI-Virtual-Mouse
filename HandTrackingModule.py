...
class HandDetector():
    ...
    def findPosition(self, img, handNo=0, draw=True):
        ...
        if self.results.multi_hand_world_landmarks:
                ...
                xmin = min(xList, default=None)
                xmax = max(xList, default=None)
    ...
    def fingersUp(self):
        fingers = []
        # Thumb
        if self.tipIds[0] < len(self.lmList) and self.tipIds[0] - 1 >= 0:
        ...
        # Fingers
        for id in range(1, 5):
            if id < len(self.tipIds) and len(self.lmList) != 0 :
